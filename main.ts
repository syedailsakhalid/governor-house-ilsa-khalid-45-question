...
let GuestList: string[] = ["Leonardo da Vinci", "Jane Austen", "Neil Armstrong"];
...
let guestWhoCantMakeIt: string = guestList[1]; // Assuming the second guest can't make it
...
let placesToVisit: string[] = ["Paris", "Saudia Arabia", "Malaysia", "Tokyo", "Greece"];
...
let Age: number = 25; // Set the age of the person
...
let new_users: string[] = ["user6", "user7", "User3", "user8", "user9"]; // Note: "User3" is in a different case
...
// Define an array of magician's names
let MagicianNames: string[] = ["famia", "anas", "a.basiit", "ayan"];
...
let Great_Magician_Names: string[] = make_great([...magicianNames]); // Use spread operator to create a copy
...
interface Car {
    manufacturer: string;
    model: string;
    [key: string]: string; // Allows arbitrary number of keyword arguments
}
